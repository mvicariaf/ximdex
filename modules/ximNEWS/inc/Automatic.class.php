...
class Automatic
{
    ...
    function checkStopper()
    {
        if (file_exists($this->stopperFilePath)) {
            $this->mutex->release();
            Logger::info("STOP: Detected file {$this->stopperFilePath}. You need to delete this file for successful restart of automatic");
    ...
    function _processColector($colectorID, $colectorName, $colectoresConFuelle, $colectorLogHead)
    {
        ...
        if (in_array($colectorID, $colectoresConFuelle)) {
            ...
            if ($now2 > $this->minHourFuelle && $now2 < $this->maxHourFuelle) {
                ...
                Logger::info($colectorLogHead . "Fuelle-less generation");
            }
        }
        ...
        if ($lockColector == 1) {
            Logger::info($colectorLogHead . "Locked (Maybe colector's being generated at this moment)");
            $generate = false;
        ...
        if ($generate == true) {
            Logger::info($colectorLogHead . "Starting generation");
            $this->checkStopper();
            $generados = $nodeColector->class->generateColector($totalGeneration);
            Logger::info($colectorLogHead . "Ending generation");
            $this->checkStopper();
        } elseif ($ximNewsColector->get('State') == 'generated') {
            Logger::info($colectorLogHead . "Bulletin already generated");
            $this->checkStopper();
        ...
        if (!empty($generados)) {
            ...
            $actualBulletin = 0;
            Logger::info($colectorLogHead . "Successfully generation ($numBulletins bulletins)");

            //Los boletines generados se publican desde ya hasta el infinito
            foreach ($generados as $bulletinID) {
                $actualBulletin++;
                Logger::info($colectorLogHead . "Publishing bulletin $bulletinID ($actualBulletin of $numBulletins)");
            ...
            $generados = array();
            Logger::info($colectorLogHead . "No generation needed");
        }
        ...
        Logger::info($colectorLogHead . "Ending processing");
        return $generados;
    }
    ...
    function _processBulletin($colectorID, $bulletinID, $colectorLogHead)
    {
        ...
        if (ModulesManager::isEnabled('ximSYNC')) {
            ...
            $actualDoc = 0;
            Logger::info($colectorLogHead . "$numDocs docs to be published");

            foreach ($this->docsToPublish as $docID) {

                $this->checkStopper();
                $actualDoc++;
                Logger::info($colectorLogHead . "Starting doc publication [$docID] ($actualDoc of $numDocs)");
